...
//            viewModel.senderbusiness_profile_status.set(it?.data?.business_profile_status)
            ...
            viewModel.firestore!!.collection("Chats").whereArrayContains("members", viewModel.senderUserID.get().toString())
                .addSnapshotListener { snapshot, error ->
                    ...
                    for(dataSnapshot in snapshot!!.documents) {
                       ...
                       val otherUserData=dataSnapshot[otherUserId] as HashMap<String,String>
...
//                        viewModel.usersList.add(LastSeenData(usderName,message as String?,userImage,milisecondTime))
                        LastMesageList.add(LastSeenData(usderName,message as String?,userImage,milisecondTime,chatID,otherUserId))
                    ...
                    viewModel.messageUserAdapter.setOnItemClick { view, position, type ->
                        when (type) {
                            "userChatDetails" -> {
                                ...
                                bundle.putString("UserName",viewModel.messageUserAdapter.getAllItems()[position].name)
                                bundle.putString("UserID",  viewModel.messageUserAdapter.getAllItems()[position].userId )
                                bundle.putString("chatID",  viewModel.messageUserAdapter.getAllItems()[position].chatId )
                                bundle.putString("userImage",viewModel.messageUserAdapter.getAllItems()[position].userImage )

                                Log.e("USERDETALS  ",viewModel.messageUserAdapter.getAllItems()[position].name + " - " +
